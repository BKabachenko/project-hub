...
function TimelineDot(props: TimelineDotProps) {
  const { asChild, className, status, ...dotProps } = props;

  const { orientation, variant } = useTimelineContext(DOT_NAME);
  const { status: contextStatus, isAlternateRight } = useTimelineItemContext(DOT_NAME);

  const DotPrimitive = asChild ? SlotPrimitive.Slot : "div";

  const finalStatus = status ?? contextStatus;
  ...
  compoundVariants: [
    {
      ...
      variant: "default",
      class:
        "start-[calc(var(--timeline-dot-size)/2-var(--timeline-connector-thickness)/2)] top-3 h-[calc(100%+0.5rem+10px)] w-[var(--timeline-connector-thickness)]",
    },
    {
      orientation: "horizontal",
      variant: "default",
...
function TimelineConnector(props: TimelineConnectorProps) {
  const { asChild, forceMount, className, status: customStatus, ...connectorProps } = props;
  ...
  const isAutoCompleted = nextItemStatus === 'completed' || nextItemStatus === 'active';

  const isConnectorCompleted = customStatus
    ? customStatus
    : isAutoCompleted;
  ...
  return (
    <ConnectorPrimitive
      ...
      data-status={customStatus ?? status}
      data-orientation={orientation}
      {...connectorProps}
      className={cn(
